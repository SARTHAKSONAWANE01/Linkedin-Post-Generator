  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { name: "Brand Profiles", val: "1 Active", desc: "Alex Dev Persona", icon: Sliders, color: "text-blue-400 bg-blue-500/10" },
          { name: "Total Post Drafts", val: drafts.length.toString(), desc: "Local saved files", icon: BookOpen, color: "text-emerald-400 bg-emerald-500/10" },
          { name: "Average Readability", val: "84%", desc: "Grade A comfort", icon: LineChart, color: "text-teal-400 bg-teal-500/10" },
          { name: "Predicted Reach", val: "High", desc: "Top 15% bracket", icon: TrendingUp, color: "text-indigo-400 bg-indigo-500/10" }
        ].map((met, idx) => {
          const Icon = met.icon;
          return (
            <div key={idx} className="glass-card rounded-2xl border border-slate-200/5 p-6 space-y-3">
              ...
              <div>
                <span className="block text-2xl font-black text-slate-100">{met.val}</span>
                <span className="block text-[10px] text-slate-400 mt-0.5">{met.desc}</span>
              </div>
            </div>
          );
        })}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="glass-card rounded-2xl border border-slate-200/10 p-6 space-y-5">
            ...
            <div className="space-y-3">
              {suggestions.map((sug, idx) => (
                <div 
                  key={idx}
                  className="flex items-center justify-between gap-4 p-4 rounded-xl bg-slate-50/60 dark:bg-slate-900/30 border border-slate-200 dark:border-slate-800 hover:border-emerald-500/20 smooth-transition"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-slate-100">{sug.title}</span>
                      <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/15">
                        {sug.category}
                      </span>
                    </div>
                  ...
                  <button
                    ...
                    className="py-2 px-3 rounded-lg bg-emerald-500/15 hover:bg-emerald-500/30 text-emerald-400 text-xs font-bold border border-emerald-500/20 smooth-transition"
                  >
                    Use Outline
                  </button>
                </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="glass-card rounded-2xl border border-slate-200/10 p-6 space-y-4">
            <div>
              <h2 className="text-lg font-bold text-slate-100">Saved Draft Library</h2>
              <p className="text-slate-400 text-xs mt-0.5">Quickly edit or copy your recent local drafts.</p>
            </div>
                ...
                {drafts.slice(0, 3).map((draft) => (
                  <div key={draft.id} className="pt-3 first:pt-0 flex items-center justify-between gap-3 group">
                    <div className="flex-1 min-w-0">
                      <span className="block text-xs font-semibold text-slate-200 truncate group-hover:text-emerald-400 smooth-transition">
                        {draft.title}
                      </span>
                    ...
                    <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition duration-150">
                      ...
                      <button
                        ...
                        className="p-1.5 rounded-lg bg-slate-900 hover:bg-red-950/20 text-slate-500 hover:text-red-400 border border-slate-800 hover:border-red-900/30"
                        title="Delete Post"
                      >
            ...
            <div className="pt-2 border-t border-slate-200/5">
              <Link 
                href="/drafts" 
                className="block w-full py-2.5 text-center text-xs font-bold text-slate-400 hover:text-emerald-400 hover:bg-slate-900/35 rounded-xl border border-slate-200/5 smooth-transition"
              >
